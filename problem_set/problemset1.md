...
**Purpose** Authenticate users through address verification to ensure identity and provide information to providers to do certain things
**Principle** After an user authenticate themselves with an address and one additional information, they can be identify to be the same individual associated with the personal information once. The additional information needed is different depending on how the different stakeholders are using the address of the users for.
**State**
Set of users:
...
**Action**
shortenURL(user:User, longURL: link, suffix: String?):(shortLink: ShortLink)
require: an existing user, existing link, if suffix is given, must be a unique name in the domain
...
deleteURL(user:User, shortURL: ShortLink)
require: an existing user that has shortURL stored in them
effect: remove the shortURL stored in the user along with the long URL associate with the shortURL. After removal, if people click through the shortURL, error webpage will popup of not being able to find the shortURL in the system.
